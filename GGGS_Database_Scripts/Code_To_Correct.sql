...
DECLARE
  ...
  -- variables v_name1 declared as gggs_stock.name%TYPE, but used to hold categoryID, 
  --type/semantic mismatch can cause runtime problems or confusion;
  --v_category_id gggs_category.categoryID%TYPE;
  --use SELECT categoryID INTO v_category_id
 
  -- it has the same problem as the v_name1
  ...
  FOR r_gggs IN c_gggs LOOP
    BEGIN 
        ...
        ELSIF (r_gggs.process_type = k_change) THEN
          UPDATE gggs_customer
             SET province = DECODE(r_gggs.column2, k_no_change_char, province, r_gggs.column2),
                 ...
                 phone_number = NVL(r_gggs.column6, r_gggs.column6, phone_number) -- should be NVL2, not NVL, NVL only allow accept two parameter
           WHERE name = r_gggs.column1;  
   	    ELSE 
      ...
      ELSIF (r_gggs.data_type = k_vendor) THEN
        ...
        ELSIF (r_gggs.process_type = k_status) THEN -- k_stats is not defined, it should use k_status
          UPDATE gggs_vendor
             SET status = r_gggs.column2
           WHERE name = r_gggs.column1;    
        ...
        ELSIF (r_gggs.process_type = k_change) THEN
          UPDATE gggs_vendor
             SET description = DECODE(r_gggs.column2, k_no_change_char, description, r_gggs.column2),
                 ...
                 contact_phone_number = NVL2(r_gggs.column6, r_gggs.column6, contact_phone_number) --here it redundantly sets the sme r_gggs.column6, the expression is confusing and unnecessarily verbose; it is intend to us NVL
           WHERE name = r_gggs.column1;  -- missing semicolon after an UPDATE ...WHERE; add semicolon
        ELSE 
	      RAISE_APPLICATION_ERROR(-20001, r_gggs.process_type || ' is not a valid process request for ' || r_gggs.data_type || ' data');
        END IF;
      ...
      ELSIF (r_gggs.data_type = k_stock) THEN
        ...
        ELSIF (r_gggs.process_type = k_change) THEN  --PL/SQL does not use ELSE IF, it use ELSIF
          UPDATE gggs_stock
    ...
    EXCEPTION 
	  ...
	  END; --add END; to close the inner BEGIN
  END LOOP;  
